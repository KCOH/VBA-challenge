...
For Each sht In Worksheets
    ...
    For i = 2 To LastRow
        ...
        'calculate Total Stock Volume by adding next value in the column to the previous one
        Dim TotalStockVolume As Double
        TotalStockVolume = TotalStockVolume + sht.Cells(i, 7)
        
        'check the new ticker symbol to the current one.
        
        If sht.Cells(i, 1).Value <> sht.Cells(i - 1, 1).Value Then
            StockOpen = sht.Cells(i, 3).Value
        End If
    
        'populate the cells that we just created
        If sht.Cells(i, 1).Value <> sht.Cells(i + 1, 1).Value Then
            'fill the ticker symbol
            sht.Cells(j, 9).Value = sht.Cells(i, 1).Value
            'fill the Total Stock Volume
            ...
            'calculate percent change but if the opening and closing stock are both 0, then it results in error.
